...
func (ec *EquipmentCreate) AddRoomIDs(ids ...int) *EquipmentCreate {
	ec.mutation.AddRoomIDs(ids...)
	return ec
}

// AddRoom adds the room edges to Room.
func (ec *EquipmentCreate) AddRoom(r ...*Room) *EquipmentCreate {
	ids := make([]int, len(r))
	for i := range r {
		ids[i] = r[i].ID
	}
	return ec.AddRoomIDs(ids...)
}
