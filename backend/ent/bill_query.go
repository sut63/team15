...
func (bq *BillQuery) QuerySituation() *SituationQuery {
	query := &SituationQuery{config: bq.config}
...
func (bq *BillQuery) QueryPayment() *PaymentQuery {
...
//  WithSituation tells the query-builder to eager-loads the nodes that are connected to
// the "Situation" edge. The optional arguments used to configure the query builder of the edge.
func (bq *BillQuery) WithSituation(opts ...func(*SituationQuery)) *BillQuery {
	query := &SituationQuery{config: bq.config}
	for _, opt := range opts {
		opt(query)
	}
	bq.withSituation = query
	return bq
}
...
func (bq *BillQuery) WithPayment(opts ...func(*PaymentQuery)) *BillQuery {
	query := &PaymentQuery{config: bq.config}
	for _, opt := range opts {
		opt(query)
	}
	bq.withPayment = query
	return bq
}
...
func (bq *BillQuery) sqlAll(ctx context.Context) ([]*Bill, error) {
	...
	if query := bq.withSituation; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Bill)
		for i := range nodes {
			if fk := nodes[i].situation_id; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "situation_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Situation = n
			}
		}
	}

	if query := bq.withPayment; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Bill)
		for i := range nodes {
