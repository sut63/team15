...
func (qq *QuantityQuery) QueryRoomdetails() *RoomdetailQuery {
	query := &RoomdetailQuery{config: qq.config}
...
func (qq *QuantityQuery) WithRoomdetails(opts ...func(*RoomdetailQuery)) *QuantityQuery {
	query := &RoomdetailQuery{config: qq.config}
	for _, opt := range opts {
		opt(query)
	}
	qq.withRoomdetails = query
	return qq
}
...
func (qq *QuantityQuery) sqlAll(ctx context.Context) ([]*Quantity, error) {
	...
	if query := qq.withRoomdetails; query != nil {
		...
		query.Where(predicate.Roomdetail(func(s *sql.Selector) {
			s.Where(sql.InValues(quantity.RoomdetailsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.quantity_roomdetails
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "quantity_roomdetails" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "quantity_roomdetails" returned %v for node %v`, *fk, n.ID)
			}
