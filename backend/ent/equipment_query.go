...
func (eq *EquipmentQuery) QueryRoom() *RoomQuery {
	query := &RoomQuery{config: eq.config}
...
func (eq *EquipmentQuery) WithRoom(opts ...func(*RoomQuery)) *EquipmentQuery {
	query := &RoomQuery{config: eq.config}
...
//		GroupBy(equipment.FieldEquipment).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Equipment.Query().
//		Select(equipment.FieldEquipment).
//		Scan(ctx, &v)
//
...
func (eq *EquipmentQuery) sqlAll(ctx context.Context) ([]*Equipment, error) {
	...
	if query := eq.withRoom; query != nil {
		...
		ids := make(map[int]*Equipment, len(nodes))
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(equipment.RoomPrimaryKey[1], fks...))
			},
