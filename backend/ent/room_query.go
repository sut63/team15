...
func (rq *RoomQuery) QueryQuantity() *QuantityQuery {
...
func (rq *RoomQuery) QueryStaytype() *StayTypeQuery {
...
func (rq *RoomQuery) QueryFacilities() *FacilityQuery {
	query := &FacilityQuery{config: rq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoomQuery) QueryEquipments() *EquipmentQuery {
...
// QueryNearbyplace chains the current query on the nearbyplace edge.
func (rq *RoomQuery) QueryNearbyplace() *NearbyPlaceQuery {
...
func (rq *RoomQuery) WithQuantity(opts ...func(*QuantityQuery)) *RoomQuery {
	query := &QuantityQuery{config: rq.config}
...
func (rq *RoomQuery) WithStaytype(opts ...func(*StayTypeQuery)) *RoomQuery {
	query := &StayTypeQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *RoomQuery) WithFacilities(opts ...func(*FacilityQuery)) *RoomQuery {
	query := &FacilityQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *RoomQuery) WithEquipments(opts ...func(*EquipmentQuery)) *RoomQuery {
	query := &EquipmentQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
...
//  WithNearbyplace tells the query-builder to eager-loads the nodes that are connected to
// the "nearbyplace" edge. The optional arguments used to configure the query builder of the edge.
func (rq *RoomQuery) WithNearbyplace(opts ...func(*NearbyPlaceQuery)) *RoomQuery {
	query := &NearbyPlaceQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withNearbyplace = query
	return rq
}
...
func (rq *RoomQuery) sqlAll(ctx context.Context) ([]*Room, error) {
	...
	if query := rq.withQuantity; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].room_quantity; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "room_quantity" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := rq.withStaytype; query != nil {
		...
		nodeids := make(map[int][]*Room)
		for i := range nodes {
			if fk := nodes[i].room_staytype; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "room_staytype" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := rq.withFacilities; query != nil {
		...
		ids := make(map[int]*Room, len(nodes))
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(room.FacilitiesPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, rq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "facilities": %v`, err)
		}
		query.Where(facility.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "facilities" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Facilities = append(nodes[i].Edges.Facilities, n)
			}
		}
	}
	...
	if query := rq.withEquipments; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(room.EquipmentsPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, rq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "equipments": %v`, err)
		}
		query.Where(equipment.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "equipments" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Equipments = append(nodes[i].Edges.Equipments, n)
			}
		}
	}

	if query := rq.withNearbyplace; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Room, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(room.NearbyplacePrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, rq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "nearbyplace": %v`, err)
		}
		query.Where(nearbyplace.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "nearbyplace" node returned %v`, n.ID)
			}
			for i := range nodes {
