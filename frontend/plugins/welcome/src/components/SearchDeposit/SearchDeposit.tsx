...
export default function Search() {
  ...
  useEffect(() => {
    ...
    const SearchDeposit = async () => {
        const res = await api.listDeposit();
		const parcelcodesearch = DeParcelcodeSearch(res);
		const leasesearch = LeaseSearch(parcelcodesearch);
        const statusdsearch = StatusdSearch(leasesearch);
        
        setErrorMessege("not found");
        setAlertType("error");
        setDeposit([]);
        if(statusdsearch.length > 0){
            Object.entries(searchcheck).map(([key, value]) =>{
                if (value == true){
                    setErrorMessege("search found");
                    setAlertType("success");
                    setDeposit(statusdsearch);
                }
            })
        }
    ...
    const ResetSearchCheck = () => {
        searchcheck.parcelcodesearchcheck = true;
        searchcheck.leasesearchcheck = true;
		searchcheck.statusdsearchcheck = true;
    }

    const DeParcelcodeSearch = (res: any) => {
        const data = res.filter((filter: EntDeposit) => filter.parcelcode?.includes(parcelcodesearch))
        //console.log(data.length)
        if (data.length != 0 && parcelcodesearch != "") {
            return data;
        }
        else {
            searchcheck.parcelcodesearchcheck = false;
            if(parcelcodesearch == ""){
                return res;
            }
            else{
    ...
    const StatusdSearch = (res: any) => {
        const data = res.filter((filter: EntDeposit) => filter.edges?.statusd?.id == statusdsearch)
        //console.log(data.length)
        if (data.length != 0) {
            return data;
        }
        else {
            searchcheck.statusdsearchcheck = false;
            if(statusdsearch == 0){
                return res;
            }
            else{
	...
	const LeaseSearch = (res: any) => {
        const data = res.filter((filter: EntDeposit) => filter.edges?.lease?.id == leasesearch)
        //console.log(data.length)
        if (data.length != 0) {
            return data;
        }
